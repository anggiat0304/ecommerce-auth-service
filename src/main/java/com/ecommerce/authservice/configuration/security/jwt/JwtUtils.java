...
public class JwtUtils {
    ...
    public boolean validateJwtToken(String authToken) {
        try {
            ...
            throw new Error("JWT token is expired: "+ e.getMessage());
        } catch (UnsupportedJwtException e) {
            logger.error("JWT token is unsupported: {}", e.getMessage());
            throw new UnsupportedJwtException("JWT token is unsupported: "+ e.getMessage());
        } catch (IllegalArgumentException e) {
            logger.error("JWT claims string is empty: {}", e.getMessage());
            throw new UnsupportedJwtException("JWT claims string is empty: "+ e.getMessage());
        }
    }
    public String extractTokenWithoutBearer(String authToken) {
        // Memisahkan "Bearer " dari token
        if (authToken != null) {
            authToken = authToken.replace("Bearer ","");// Mengambil token setelah "Bearer "
            return authToken;
        }
        return authToken;
    }
